...
def load_payments(df, engine, logger=None):
    ...
    try:
        ...
        if logger:
            logger.info(
                f"Loading {len(df)} rows into staging load table"
            )
        ...
        if logger:
            logger.info(
                "Data loaded into staging load table"
            )

        # -------------------------------
        # STEP 3: MERGE WITH ORDER JOIN
        # -------------------------------
        ...
        if logger:
            logger.error(
                f"Error during payments MERGE load: {e}"
            )
