...
logger = get_logger("payments_pipeline", "payments_pipeline.log")


def run_pipeline():
    logger.info("Starting payments pipeline")
    ...
    try:
        ...
        if response["status"] != "success":
            raise Exception("Payments API failed after maximum retries")
        ...
        # VALIDATION
        # -------------------------------
        df = validate_payments(raw_data, logger)
        ...
        # -------------------------------
        watermark = get_watermark(engine, "payments_pipeline")
        ...
        df = df[df["payment_date"] > adjusted_watermark]
        ...
        # LOAD
        # -------------------------------
        load_payments(df, engine, logger)
        ...
        # UPDATE WATERMARK
        # -------------------------------
        new_watermark = df["payment_date"].max()

        update_watermark(engine, "payments_pipeline", new_watermark)

        logger.info(f"Watermark updated to: {new_watermark}")

        logger.info("Payments pipeline completed successfully")

    except Exception as e:
        logger.exception(f"Payments pipeline failed: {e}")
        raise
